...
//new BEST PRICE 04/05/2014, fabio voleva l'ultimo prezzo netto applicato
// $bestprice="SELECT MIN(distinct(CAST(replace(A.newprice,',','.')As DEC(7,3)))) from 
// `ordine_cliente_prod` A
// join ordine O
// where id_utente_fk= '{$_SESSION['loggedid']}'
// and codice_prodotto ='{$prodbought['codice_prodotto']}' and A.newprice is not null and A.newprice <> '' and A.codice_ordine < {$_GET['conto']}
// group by A.codice_ordine
// order by A.codice_ordine DESC
// LIMIT 1";
...
header('Content-type: text/xml');
// echo $xml->asXML();
$xmlheader="<?phpxml version=\"1.0\" encoding=\"UTF-8\"?>";
