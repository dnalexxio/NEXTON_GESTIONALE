...
function drillon(){
 ...
 dsSeek.setURL('file_xml/newrighe.php?usern=<?php echo $_SESSION['logged'];?>&conto='+contonumero);
 dsTotale.setURL('file_xml/newrighe.php?usern=<?php echo $_SESSION['logged'];?>&conto='+contonumero);
dsSeek.loadData();
dsTotale.loadData();
...
<div id="stile" ><?=$gestore['ragione_sociale']?><br><?=$gestore['telefono']?><br><?=$gestore['email']?></div>
...
<div id="Special_DIV" spry:region="dsAcquisti">
	<table id="products">
		<caption> Riepilogo dell'acquisto della ditta <?php echo $_SESSION['logged'];?> <font color="#FF2B0A">ORDINE NUMERO {trackno}({codiceord})</font></caption>
		<tr>
