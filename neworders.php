...
<link href="newstyle.css" rel="stylesheet" type="text/css" media="(min-device-width: 769px)" >
		...
		<script language="JavaScript" type="text/javascript">
			...
			var dsUser = new Spry.Data.XMLDataSet("file_xml/anagrafica.php?usern=<?php echo $_SESSION['logged'];?>", "voce");
			var dsAcquisti = new Spry.Data.XMLDataSet("file_xml/newrighe.php?usern=<?php echo $_SESSION['logged'];?>&conto=<?php echo $_SESSION['conto'];?>", "ordini/prodotto", {   useCache:  false , sortOnLoad: "ds_RowID"  });
			
			var dsTotale = new Spry.Data.XMLDataSet("file_xml/newrighe.php?usern=<?php echo $_SESSION['logged'];?>&conto=<?php echo $_SESSION['conto'];?>", "ordini/riepilogo", {   useCache:  false   });

dsAcquisti.setColumnType("ds_RowID", "number");
function drillon(){

dsAcquisti.setURL('file_xml/newrighe.php?usern=<?php echo $_SESSION['logged'];?>&conto=<?php echo $_SESSION['conto'];?>',{ useCache:  false },{ sortOnLoad: "ds_RowID" });
  dsTotale.setURL('file_xml/newrighe.php?usern=<?php echo $_SESSION['logged'];?>&conto=<?php echo $_SESSION['conto'];?>',{ useCache:  false },{ sortOnLoad: "code" });
dsAcquisti.loadData();
 dsTotale.loadData();
...
0804912083 <br>GLBFBA78E11H096U<br>p.iva: IT05524720728
--!>

<?=$gestore['ragione_sociale']?><br><?=$gestore['telefono']?><br><?=$gestore['email']?>
</div>
<div id="client" spry:region="dsUser" align="right">Ordine della DITTA:<br> {cognome} <br>{indirizzo}<br> {citta} {cap}<br>Tel {telefono} - FAX:{nazione}<br>{paga} ABI/CAB:{abicab}<br> {cfisc}<br>{mail}
		...
		<br>
		<a onclick="dojo.widget.byId('docpane').setUrl('note.php?prova=<?php echo $_SESSION['conto'];?>');" class="stampa">inserisci note</a>
		</div>
...
<?php echo $_SESSION['conto']."   (".$_SESSION['contovisualizzabile'].")  "; ?> 
		</caption>
		<tr>
		...
		<tbody spry:repeatchildren="dsAcquisti" >
			<tr  
				...
				<td width="10%"><center><button name="aa" class="stampa" onclick="dojodel3('<?php echo $_SESSION['conto'];?>','{id_prod}', '{misura}')">cancella</button></center></td>
			</tr>
...
<input type="hidden" name="total_sped" value="<?php echo $_SESSION['conto']; ?>"> 
...
<div id="noteperm" spry:region="dsUser">
<p align="left" name="notaperm" class="stampa">Nota permanente: {notapermanente}</p>
</div>
