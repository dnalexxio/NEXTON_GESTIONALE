...
if($pass=="" || $name=="") { header("index.php",1); echo("La combinazione di credenziali è invalida");  die("<a href=index.php> Riprova</a>");}
...
$sql = "SELECT id,password FROM utente WHERE utente='{$name}'";
 $result=$dbh->query($sql);
    ...
    if($result['password']==$pass) {
			...
			echo "<div id=\"div_user\">{$_SESSION['logged']},(codice cliente {$_SESSION['loggedid']})<br> </div>";
...
$dbh=connect();
register($_REQUEST['username'],$_REQUEST['passwd'],$_REQUEST['email'],$_REQUEST['codice_controllo']);
...
session_start();
$link = mysql_connect("localhost", "root", "oZdij5");
$database = mysql_select_db("fabio"); 
...
echo("<h1><font color=red>Errore<br></font></h1>");
